...
class MyDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        ...
        try:
            ...
            temp = round(
                int.from_bytes(data[0:2], byteorder="little", signed=True) / 100, 1
            )
        ...
        with self._instance_lock:
            ...
            while not success and retries < 3:
                ...
                try:
                    ...
                    _LOGGER.error(
                        "%s Connection lost, Retrying ... %d" % (mac, retries)
                    )
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    if not config_entry.data and not sensors:
        return False
    ...
    def discover_ble_device(config):
        ...
        if data:
            for dev in devs:
                ...
                _LOGGER.debug(
                    "important %s %s state updated.", config[CONF_MAC], device_class
                )
